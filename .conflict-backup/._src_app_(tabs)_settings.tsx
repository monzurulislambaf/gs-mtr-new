import { useState, useEffect } from 'react';
<<<<<<< HEAD
import { View, StyleSheet, ScrollView, Linking, Pressable } from 'react-native';
import {
  Text,
=======
import { View, StyleSheet, ScrollView } from 'react-native';
import {
  Text,
  Switch,
>>>>>>> 976be18251497c7c9549b752f4e9178f5f669dd0
  Button,
  List,
  Divider,
  useTheme,
  Dialog,
  Portal,
  Snackbar,
  RadioButton,
} from 'react-native-paper';
import { SafeAreaView } from 'react-native-safe-area-context';
import { router } from 'expo-router';
import { useAuthStore } from '@/store/authStore';
<<<<<<< HEAD
import { useSettingsStore } from '@/store/settingsStore';
import { useContactsStore } from '@/store/contactsStore';
import { getAllContactsCount } from '@/database/database';
import { exportContactsToCsv, bulkImportFromCsv } from '@/services/csvService';
import { bulkImportContacts } from '@/firebase/firestore';
import { APP_NAME, APP_VERSION } from '@/utils/constants';
import { HeaderActions } from '@/components/ui/HeaderActions';
=======
import { useSyncStore } from '@/store/syncStore';
import { useSettingsStore } from '@/store/settingsStore';
import { getDatabaseSize, getAllContactsCount, clearAllContacts } from '@/database/database';
import { exportContactsToCsv, bulkImportFromCsv } from '@/services/csvService';
import { bulkImportContacts, seedDemoContacts } from '@/firebase/firestore';
import { APP_NAME, APP_VERSION } from '@/utils/constants';
>>>>>>> 976be18251497c7c9549b752f4e9178f5f669dd0

export default function SettingsScreen() {
  const theme = useTheme();
  const user = useAuthStore((s) => s.user);
<<<<<<< HEAD
  const isAdmin = useAuthStore((s) => s.isAdmin);
  const logout = useAuthStore((s) => s.logout);
  const { theme: themeMode, setTheme } = useSettingsStore();
  const reloadContacts = useContactsStore((s) => s.loadContacts);

  const [contactCount, setContactCount] = useState(0);
  const [showThemeDialog, setShowThemeDialog] = useState(false);
  const [showLogoutDialog, setShowLogoutDialog] = useState(false);
  const [importing, setImporting] = useState(false);
  const [exporting, setExporting] = useState(false);
  const [snackbar, setSnackbar] = useState({ visible: false, message: '' });

  useEffect(() => {
=======
  const logout = useAuthStore((s) => s.logout);
  const { isSyncing, lastSyncTime, manualSync, isOnline } = useSyncStore();
  const { autoSync, setAutoSync, theme: themeMode, setTheme } = useSettingsStore();

  const [dbSize, setDbSize] = useState('...');
  const [contactCount, setContactCount] = useState(0);
  const [showThemeDialog, setShowThemeDialog] = useState(false);
  const [showLogoutDialog, setShowLogoutDialog] = useState(false);
  const [showClearDialog, setShowClearDialog] = useState(false);
  const [snackbar, setSnackbar] = useState({ visible: false, message: '' });

  useEffect(() => {
    getDatabaseSize().then(setDbSize);
>>>>>>> 976be18251497c7c9549b752f4e9178f5f669dd0
    getAllContactsCount().then(setContactCount);
  }, []);

  function showSnackbar(message: string) {
    setSnackbar({ visible: true, message });
  }

<<<<<<< HEAD
  async function handleExport() {
    setExporting(true);
=======
  async function handleSync() {
    try {
      await manualSync();
      showSnackbar('Sync completed');
    } catch (e: any) {
      showSnackbar(e.message || 'Sync failed');
    }
  }

  async function handleExport() {
>>>>>>> 976be18251497c7c9549b752f4e9178f5f669dd0
    try {
      await exportContactsToCsv();
      showSnackbar('Contacts exported');
    } catch (e: any) {
      showSnackbar(e.message || 'Export failed');
<<<<<<< HEAD
    } finally {
      setExporting(false);
=======
>>>>>>> 976be18251497c7c9549b752f4e9178f5f669dd0
    }
  }

  async function handleImport() {
<<<<<<< HEAD
    setImporting(true);
    try {
      const result = await bulkImportFromCsv(bulkImportContacts);
      await reloadContacts();
      getAllContactsCount().then(setContactCount);
      showSnackbar(`Imported: ${result.imported}, Skipped: ${result.skipped}`);
    } catch (e: any) {
      showSnackbar(e.message || 'Import failed');
    } finally {
      setImporting(false);
    }
  }

=======
    try {
      const result = await bulkImportFromCsv(bulkImportContacts);
      showSnackbar(`Imported: ${result.imported}, Skipped: ${result.skipped}`);
    } catch (e: any) {
      showSnackbar(e.message || 'Import failed');
    }
  }

  async function handleSeed() {
    try {
      const count = await seedDemoContacts();
      showSnackbar(`${count} demo contacts created. Pull to refresh.`);
    } catch (e: any) {
      showSnackbar(e.message || 'Failed to create demo data');
    }
  }

  async function handleClearCache() {
    await clearAllContacts();
    setContactCount(0);
    setShowClearDialog(false);
    showSnackbar('Local cache cleared. Pull to refresh to re-download.');
  }

>>>>>>> 976be18251497c7c9549b752f4e9178f5f669dd0
  async function handleLogout() {
    await logout();
    setShowLogoutDialog(false);
    showSnackbar('Signed out successfully');
  }

  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={styles.header}>
        <Text variant="headlineMedium" style={{ fontWeight: '600' }}>
          Settings
        </Text>
<<<<<<< HEAD
        <HeaderActions />
=======
>>>>>>> 976be18251497c7c9549b752f4e9178f5f669dd0
      </View>

      <ScrollView contentContainerStyle={styles.scroll}>
        <List.Section>
          <List.Subheader style={{ color: theme.colors.primary }}>
            Account
          </List.Subheader>
          {user ? (
            <>
              <List.Item
                title={user.displayName || user.email || ''}
                description={user.email}
                left={(props) => <List.Icon {...props} icon="account-circle" />}
              />
            </>
          ) : (
            <List.Item
              title="Sign In"
              description="Sign in for full access"
              left={(props) => <List.Icon {...props} icon="login" />}
              onPress={() => router.push('/(auth)/login' as any)}
            />
          )}
        </List.Section>

        <Divider />

        <List.Section>
          <List.Subheader style={{ color: theme.colors.primary }}>
            Appearance
          </List.Subheader>
          <List.Item
            title="Theme"
            description={themeMode === 'system' ? 'System' : themeMode === 'dark' ? 'Dark' : 'Light'}
            left={(props) => <List.Icon {...props} icon="theme-light-dark" />}
            onPress={() => setShowThemeDialog(true)}
          />
        </List.Section>

        <Divider />

        <List.Section>
          <List.Subheader style={{ color: theme.colors.primary }}>
            Synchronization
          </List.Subheader>
          <List.Item
<<<<<<< HEAD
=======
            title="Auto Sync"
            description="Automatically sync when online"
            left={(props) => <List.Icon {...props} icon="sync" />}
            right={() => (
              <Switch
                value={autoSync}
                onValueChange={setAutoSync}
                color={theme.colors.primary}
              />
            )}
          />
          <List.Item
            title="Manual Sync"
            description={isSyncing ? 'Syncing...' : lastSyncTime ? `Last sync: ${new Date(lastSyncTime).toLocaleString()}` : 'Never synced'}
            left={(props) => <List.Icon {...props} icon="cloud-sync" />}
            onPress={handleSync}
            disabled={isSyncing || !isOnline}
          />
          <List.Item
>>>>>>> 976be18251497c7c9549b752f4e9178f5f669dd0
            title="Synced Contacts"
            description={`${contactCount} contacts`}
            left={(props) => <List.Icon {...props} icon="contacts" />}
          />
<<<<<<< HEAD
        </List.Section>

        {isAdmin && (
=======
          <List.Item
            title="Database Size"
            description={dbSize}
            left={(props) => <List.Icon {...props} icon="database" />}
          />
        </List.Section>

        {user && (
>>>>>>> 976be18251497c7c9549b752f4e9178f5f669dd0
          <>
            <Divider />

            <List.Section>
              <List.Subheader style={{ color: theme.colors.primary }}>
                Data Management
              </List.Subheader>
              <List.Item
                title="Export Contacts (CSV)"
                description="Download all contacts as CSV file"
                left={(props) => <List.Icon {...props} icon="file-export" />}
                onPress={handleExport}
<<<<<<< HEAD
                disabled={exporting}
=======
>>>>>>> 976be18251497c7c9549b752f4e9178f5f669dd0
              />
              <List.Item
                title="Import Contacts (CSV)"
                description="Upload contacts from CSV file"
                left={(props) => <List.Icon {...props} icon="file-import" />}
                onPress={handleImport}
<<<<<<< HEAD
                disabled={importing}
=======
              />
              <List.Item
                title="Load Demo Data"
                description="Insert 25 sample contacts for testing"
                left={(props) => <List.Icon {...props} icon="database-plus" />}
                onPress={handleSeed}
>>>>>>> 976be18251497c7c9549b752f4e9178f5f669dd0
              />
            </List.Section>
          </>
        )}
<<<<<<< HEAD
=======
        <Divider />

        <List.Section>
          <List.Subheader style={{ color: theme.colors.primary }}>
            Storage
          </List.Subheader>
          <List.Item
            title="Clear Local Cache"
            description="Remove all locally stored contacts"
            left={(props) => <List.Icon {...props} icon="delete-sweep" />}
            onPress={() => setShowClearDialog(true)}
          />
        </List.Section>
>>>>>>> 976be18251497c7c9549b752f4e9178f5f669dd0

        <Divider />

        <List.Section>
          <List.Subheader style={{ color: theme.colors.primary }}>
            About
          </List.Subheader>
          <List.Item
            title={APP_NAME}
            description={`Version ${APP_VERSION}`}
            left={(props) => <List.Icon {...props} icon="information" />}
          />
<<<<<<< HEAD
          <List.Item
            title="Developed by"
            description="BD/470504 Sgt Monzurul GS"
            left={(props) => <List.Icon {...props} icon="account-tie" />}
          />
          <List.Item
            title="Email"
            description="monzurulislambaf@gmail.com"
            left={(props) => <List.Icon {...props} icon="email" />}
            onPress={() => Linking.openURL('mailto:monzurulislambaf@gmail.com')}
          />
          <List.Item
            title="GitHub"
            description="github.com/monzurulislambaf"
            left={(props) => <List.Icon {...props} icon="github" />}
            onPress={() => Linking.openURL('https://github.com/monzurulislambaf')}
          />
=======
>>>>>>> 976be18251497c7c9549b752f4e9178f5f669dd0
        </List.Section>

        {user && (
          <>
            <Divider />
            <View style={styles.logoutSection}>
              <Button
                mode="outlined"
                onPress={() => setShowLogoutDialog(true)}
                textColor={theme.colors.error}
                style={styles.logoutButton}
              >
                Sign Out
              </Button>
            </View>
          </>
        )}
      </ScrollView>

      <Portal>
        <Dialog visible={showThemeDialog} onDismiss={() => setShowThemeDialog(false)}>
          <Dialog.Title>Choose Theme</Dialog.Title>
          <Dialog.Content>
            <RadioButton.Group
              onValueChange={(v) => {
                setTheme(v as any);
                setShowThemeDialog(false);
              }}
              value={themeMode}
            >
              <RadioButton.Item label="System" value="system" />
              <RadioButton.Item label="Light" value="light" />
              <RadioButton.Item label="Dark" value="dark" />
            </RadioButton.Group>
          </Dialog.Content>
        </Dialog>

        <Dialog visible={showLogoutDialog} onDismiss={() => setShowLogoutDialog(false)}>
          <Dialog.Title>Sign Out</Dialog.Title>
          <Dialog.Content>
            <Text>Are you sure you want to sign out?</Text>
          </Dialog.Content>
          <Dialog.Actions>
            <Button onPress={() => setShowLogoutDialog(false)}>Cancel</Button>
            <Button onPress={handleLogout} textColor={theme.colors.error}>Sign Out</Button>
          </Dialog.Actions>
        </Dialog>

<<<<<<< HEAD
=======
        <Dialog visible={showClearDialog} onDismiss={() => setShowClearDialog(false)}>
          <Dialog.Title>Clear Cache</Dialog.Title>
          <Dialog.Content>
            <Text>This will remove all locally stored contacts. You will need to re-sync when online.</Text>
          </Dialog.Content>
          <Dialog.Actions>
            <Button onPress={() => setShowClearDialog(false)}>Cancel</Button>
            <Button onPress={handleClearCache} textColor={theme.colors.error}>Clear</Button>
          </Dialog.Actions>
        </Dialog>

>>>>>>> 976be18251497c7c9549b752f4e9178f5f669dd0
        <Snackbar
          visible={snackbar.visible}
          onDismiss={() => setSnackbar({ ...snackbar, visible: false })}
          duration={3000}
        >
          {snackbar.message}
        </Snackbar>
      </Portal>
    </SafeAreaView>
  );
}

const styles = StyleSheet.create({
  container: { flex: 1 },
  header: {
    paddingHorizontal: 16,
    paddingTop: 8,
    paddingBottom: 4,
  },
  scroll: {
    paddingBottom: 80,
  },
  logoutSection: {
    padding: 24,
    alignItems: 'center',
  },
  logoutButton: {
    borderRadius: 24,
    minWidth: 200,
  },
});
